 ...
 $DIALOG --clear --title " Cyb3rTools Features Menu " "$@" --menu \
  ...
  case $DIALOG_RESULT in
    ...
    "RESTART")
      ...
      exit 0;;
    "STOP SERVICES")
      process "systemctl stop anki_robot.target" "Stopping anki_robot.target";menu_features;;
    "START SERVICES")
      process "systemctl start anki_robot.target" "Starting anki_robot.target";menu_features;;
  esac
}
